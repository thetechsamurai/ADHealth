...
.DESCRIPTION
Combined Active Directory Health and Security Report.
Generates a multi-tab HTML report covering Forest, Domains, Domain Controllers, Security, Hardware/Software, and a Security Check tab.

.OUTPUTS
Report and log saved to: C:\ADHealthReport\
...
param (
    $Clear,
    [int]$JobTimeout    = 180,
    [int]$UserLogonAge  = 180,   # Days since last logon before a user is considered inactive
    [int]$UserPasswordAge = 180  # Days since last password set before considered stale
)

Write-Host 'Starting ADHealthReport Script..' -ForegroundColor Green
...
if ((Test-Path -Path C:\ADHealthReport -PathType Container) -eq $false) {New-Item -Type Directory -Force -Path C:\ADHealthReport}

$Global:report = ("C:\ADHealthReport\ADHealthReport_Report_"+(get-date -Format 'yyyy-MM-dd-hh-mm')+".htm") 
if ((test-path $report) -eq $false) {new-item $report -Type file -Force}
Clear-Content $report 

$Global:ADHRLog = "C:\ADHealthReport\ADHealthReport.log"
if ((test-path $ADHRLog) -eq $false) {new-item $ADHRLog -Type file -Force}
Clear-Content $ADHRLog 

Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting ADHealthReport Script")
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Data Catcher")
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Setting Error Action Preference")
...
#--------------------------------------------------------------------------------[Import AD Module & Auto-Detect DC]-------------------------------------------------------------------------
...
try {
    ...
    Write-Host 'Failed to import the Active Directory module. Ensure RSAT is installed.' -ForegroundColor Red
    Break
}

# Auto-detect PDC Emulator as the DC to connect to
$DCtoConnect = (Get-ADDomainController -Discover -Service PrimaryDC -ErrorAction SilentlyContinue).Hostname
if ([string]::IsNullOrEmpty($DCtoConnect)) {
    $DCtoConnect = $env:LOGONSERVER.TrimStart('\')
}
#--------------------------------------------------------------------------------[Begin of Functions]-------------------------------------------------------------------------

#-----------------------------------------[Header]--------------------------------------------------------

Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Selecting Script Option")
Write-Host ""
...
Write-Host ""
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Option: "+$Global:Option+" Selected")
...
function Hammer 
    {
        ...
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting The Hammer!")
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Creating Hammer Folder")


        if ((Test-Path -Path C:\ADHealthReport\Hammer -PathType Container) -eq $false) {New-Item -Type Directory -Force -Path C:\ADHealthReport\Hammer}

        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Cleaning current Powershell Job History")

        Get-Job | Remove-Job

        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Calling DCDiag")

        if($Clear.isPresent)
            {
                $Files = Get-ChildItem -path 'C:\ADHealthReport\Hammer\' 
        ...
        function HammerForest 
            {
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Active Directory RecycleBin Check")
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Duplicated SPNs check")

                Start-job -Name 'SPN' -scriptblock {setspn -X -F} | Out-Null

                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Trusts Inventory")
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Inventory")
        ...
        function HammerDomain
            {
                ...
                Foreach ($Domain in $Global:Domains)
                    { 
                        ...
                        start-job -Name ($Domain.Name+'_GPOs') -scriptblock {Get-GPOReport -All -ReportType XML -Path ("C:\ADHealthReport\Hammer\GPOs_"+$args+".xml")} -ArgumentList $Domain.Name | Out-Null
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Inventory")
            }        
        ...
        function HammerDC
            {
                ...
                #start-job -Name ($DC.Name+'_CleartxtEvt') -scriptblock {(Get-EventLog -LogName Security -InstanceId 4624 -Message '*Logon Type:			8*' -ComputerName $args).Count} -ArgumentList $DC.Name | Out-Null
                    ...
                    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Inventory of: "+$DC.Name+'. On: '+$DC.Domain)
                    ...
                    $GPResult = ([PowerShell]::Create()).AddScript({param($DomControl)Get-GPResultantSetOfPolicy -Computer $DomControl -ReportType Xml -Path ("C:\ADHealthReport\Hammer\RSOP_"+$DomControl+".xml")}).AddArgument($($args[0]))
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Waiting Inventories Conclusion")
            }        
                ...
                while (get-job | Where-Object {$_.State -eq 'Running'})
                    {
                        ...
                        if ((New-TimeSpan -Start $WaitTime2 -End (get-date)).TotalMinutes -ge 10)
                            {
                                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Still Waiting for the following Inventory Jobs:")
                                foreach($jbb in ($jb | Where-Object {$_.State -eq 'Running'}))
                                    {
                                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Job: "+$jbb.Name)
                                    }
                                $WaitTime2 = get-date
                            }
                        if ((New-TimeSpan -Start $WaitTime -End (get-date)).TotalMinutes -ge $JobTimeout)
                            {
                                ...
                                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Timing Out Inventory Jobs")
                            }
                        Start-Sleep -Seconds 2
                    }
                Write-Progress -activity 'Running Inventories' -Status "100% Complete." -Completed

                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - All Inventories are completed")
        ...
        function ForestJob
            {
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Forest Inventory")
                ...
                if ((test-path 'C:\ADHealthReport\Hammer\Forest.xml') -eq $true) {remove-item -Path 'C:\ADHealthReport\Hammer\Forest.xml' -Force}
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Registering Forest XML File")

                $Fores | Export-Clixml -Path 'C:\ADHealthReport\Hammer\Forest.xml'
        ...
        function DomainJob 
            {    
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Domain Inventory")
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Domains Details")
                ...
                Foreach ($Domain in $Global:Domains)
                    {
                        ...
                        Start-job -Name ($Domain.Name+'_job') -ScriptBlock {                            
                            if((test-path ('C:\ADHealthReport\Hammer\Domain_'+$($args[0]).Name+'.xml')) -eq $true -and $($args[9]) -ne 2) 
                                {
                                    remove-item -Path ('C:\ADHealthReport\Hammer\Domain_'+$($args[0]).Name+'.xml') -Force
                                }
                            ...
                            $DomainTable | Export-Clixml -Path ('C:\ADHealthReport\Hammer\Domain_'+$($args[0]).Name+'.xml')
        ...
        function DCjob 
            {
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Domain Controllers Inventory")
                ...
                Foreach ($DC in $Global:DCs)
                    {
                        ...
                        Start-job -Name ('Job_'+$DC.Name) -ScriptBlock {
                            if((test-path ("C:\ADHealthReport\Hammer\Inv_"+$($args[0]).Name+".xml")) -eq $true -and $($args[3]) -ne 2) 
                                {
                                    remove-item -Path ("C:\ADHealthReport\Hammer\Inv_"+$($args[0]).Name+".xml") -Force
                                }
                            ...
                            $DomControl | Export-Clixml -Path ('C:\ADHealthReport\Hammer\Inv_'+$($args[0]).Name+'.xml')
                ...
                while (get-job | Where-Object {$_.State -eq 'Running'})
                    {
                        ...
                        if ((New-TimeSpan -Start $WaitTime2 -End (get-date)).TotalMinutes -ge 10)
                            {
                                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Still Waiting for the following Processing Jobs:")
                                foreach($jbb in ($jb | Where-Object {$_.State -eq 'Running'}))
                                    {
                                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Job: "+$jbb.Name)
                                    }
    ...
    {
    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - The following Inventory Job Failed: "+$jbName)
    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - With the following Command Line: "+$jbcmd)
    }
    $time = New-TimeSpan -Start $JB.PSBeginTime -End $JB.PSEndTime
    $TimeJobMin = $time.TotalMinutes.ToString('#######.##')
    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - The following Inventory Job: "+$jbName+'. run time was: '+$TimeJobMin+' Minutes.')
    }
    ...
    $DomainControllersFolder = Get-ChildItem -Path 'C:\ADHealthReport\Hammer\' -Recurse
    ...
    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Hammer")
...
#----------------------------------------[SecurityCheckReport Function]---------------------------------------------------

function SecurityCheckReport {

    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Security Check Report")
    Write-Host 'Generating Security Check tab..' -ForegroundColor Cyan

    Add-Content $report "       <div id='SecurityCheck' class='tabcontent'>"
    Add-Content $report "           <section>"
    Add-Content $report "               <h2>Security Check<hr></h2>"
    Add-Content $report "               <p>This section provides a focused security check of the Active Directory environment, covering domain configuration, user account security, password policy, tombstone, Kerberos delegation, SYSVOL GPP passwords, KRBTGT account, and privileged groups.</p>"
    Add-Content $report "           </section>"
    ...
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Collecting Domain Info")
        $SCForestInfo  = Get-ADForest  -Server $DCtoConnect -ErrorAction Stop
        $SCDomainInfo  = Get-ADDomain  -Server $DCtoConnect -ErrorAction Stop

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Domain Information</h3>"
        Add-Content $report "               <table width='50%' align='center' border='1' class='vertical-table'>"
        Add-Content $report "                   <tr><th>ForestName</th><td>$($SCForestInfo.Name)</td></tr>"
        Add-Content $report "                   <tr><th>DomainName</th><td>$($SCDomainInfo.DNSRoot)</td></tr>"
        Add-Content $report "                   <tr><th>ForestMode (FFL)</th><td>$($SCForestInfo.ForestMode)</td></tr>"
        Add-Content $report "                   <tr><th>DomainMode (DFL)</th><td>$($SCDomainInfo.DomainMode)</td></tr>"
        Add-Content $report "                   <tr><th>SchemaMaster</th><td>$($SCForestInfo.SchemaMaster)</td></tr>"
        Add-Content $report "                   <tr><th>DomainNamingMaster</th><td>$($SCForestInfo.DomainNamingMaster)</td></tr>"
        Add-Content $report "                   <tr><th>PDCEmulator</th><td>$($SCDomainInfo.PDCEmulator)</td></tr>"
        Add-Content $report "                   <tr><th>RIDMaster</th><td>$($SCDomainInfo.RIDMaster)</td></tr>"
        Add-Content $report "                   <tr><th>InfrastructureMaster</th><td>$($SCDomainInfo.InfrastructureMaster)</td></tr>"
        Add-Content $report "               </table>"
        Add-Content $report "           </CENTER>"
    }
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Domain Info failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Domain Info collection failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 2 - Domain Users Summary
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Collecting Domain Users")
        $SCLastLogonDate     = (Get-Date).AddDays(-$UserLogonAge)
        $SCPasswordStaleDate = (Get-Date).AddDays(-$UserPasswordAge)
        $SCUserProps = @("Name","Enabled","SAMAccountname","LastLogonDate","PasswordLastSet","PasswordNeverExpires","PasswordNotRequired","DoesNotRequirePreAuth","SIDHistory","UserAccountControl")
        [array]$SCAllUsers         = Get-ADUser -Filter * -Properties $SCUserProps -Server $DCtoConnect -ErrorAction Stop
        [array]$SCEnabled          = $SCAllUsers | Where-Object { $_.Enabled -eq $true }
        [array]$SCDisabled         = $SCAllUsers | Where-Object { $_.Enabled -eq $false }
        [array]$SCInactive         = $SCEnabled  | Where-Object { ($_.LastLogonDate -le $SCLastLogonDate) -and ($_.PasswordLastSet -le $SCPasswordStaleDate) }
        [array]$SCPwdNeverExpires  = $SCAllUsers | Where-Object { $_.PasswordNeverExpires -eq $true }
        [array]$SCSIDHistory       = $SCAllUsers | Where-Object { $_.SIDHistory -like "*" }
        [array]$SCRevEnc           = $SCAllUsers | Where-Object { $_.UserAccountControl -band 0x0080 }
        [array]$SCPwdNotReq        = $SCAllUsers | Where-Object { $_.PasswordNotRequired -eq $true }
        [array]$SCKerbDES          = $SCAllUsers | Where-Object { $_.UserAccountControl -band 0x200000 }
        [array]$SCNoPreAuth        = $SCAllUsers | Where-Object { $_.DoesNotRequirePreAuth -eq $true }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Domain Users Summary</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Metric</th><th>Count</th></tr>"

        # Neutral counts
        Add-Content $report "                   <tr><td>Total Users</td><td align='center'>$($SCAllUsers.Count)</td></tr>"
        Add-Content $report "                   <tr><td>Enabled Users</td><td align='center'>$($SCEnabled.Count)</td></tr>"
        Add-Content $report "                   <tr><td>Disabled Users</td><td align='center'>$($SCDisabled.Count)</td></tr>"
        Add-Content $report "                   <tr><td>Inactive Users (logon+pwd both stale > $($UserLogonAge) days)</td><td align='center'>$($SCInactive.Count)</td></tr>"
        Add-Content $report "                   <tr><td>Password Never Expires</td><td align='center'>$($SCPwdNeverExpires.Count)</td></tr>"
        Add-Content $report "                   <tr><td>SID History Present</td><td align='center'>$($SCSIDHistory.Count)</td></tr>"

        # Color-coded fail rows
        if ($SCRevEnc.Count -gt 0) {
            $tempnames = ($SCRevEnc | ForEach-Object { $_.SamAccountName }) -join '<br>'
            Add-Content $report "                   <tr><td>Reversible Encryption Enabled</td><td bgcolor='$TableErrorColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$tempnames')"">$($SCRevEnc.Count)</a></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>Reversible Encryption Enabled</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }

        if ($SCPwdNotReq.Count -gt 0) {
            $tempnames = ($SCPwdNotReq | ForEach-Object { $_.SamAccountName }) -join '<br>'
            Add-Content $report "                   <tr><td>Password Not Required</td><td bgcolor='$TableErrorColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$tempnames')"">$($SCPwdNotReq.Count)</a></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>Password Not Required</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }

        if ($SCKerbDES.Count -gt 0) {
            $tempnames = ($SCKerbDES | ForEach-Object { $_.SamAccountName }) -join '<br>'
            Add-Content $report "                   <tr><td>Kerberos DES Enabled</td><td bgcolor='$TableErrorColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$tempnames')"">$($SCKerbDES.Count)</a></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>Kerberos DES Enabled</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }

        if ($SCNoPreAuth.Count -gt 0) {
            $tempnames = ($SCNoPreAuth | ForEach-Object { $_.SamAccountName }) -join '<br>'
            Add-Content $report "                   <tr><td>No Kerberos Pre-Authentication</td><td bgcolor='$TableErrorColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$tempnames')"">$($SCNoPreAuth.Count)</a></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>No Kerberos Pre-Authentication</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Domain Users failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Domain Users collection failed: $($_.Exception.Message)</p>"
    }
    ...
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Collecting Password Policy")
        $SCPwdPolicy = Get-ADDefaultDomainPasswordPolicy -Server $DCtoConnect -ErrorAction Stop

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Default Domain Password Policy</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Setting</th><th>Value</th><th>Status</th></tr>"

        # ComplexityEnabled
        if ($SCPwdPolicy.ComplexityEnabled -ne $true) {
            Add-Content $report "                   <tr><td>ComplexityEnabled</td><td align='center'>$($SCPwdPolicy.ComplexityEnabled)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>ComplexityEnabled</td><td align='center'>$($SCPwdPolicy.ComplexityEnabled)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # LockoutDuration (fail if < 15 min)
        $SCLockDur = $SCPwdPolicy.LockoutDuration.TotalMinutes
        if ($SCLockDur -lt 15) {
            Add-Content $report "                   <tr><td>LockoutDuration</td><td align='center'>$($SCPwdPolicy.LockoutDuration)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL (< 15 min)</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>LockoutDuration</td><td align='center'>$($SCPwdPolicy.LockoutDuration)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # MaxPasswordAge (fail if > 60 days)
        $SCMaxPwdDays = $SCPwdPolicy.MaxPasswordAge.TotalDays
        if ($SCMaxPwdDays -gt 60) {
            Add-Content $report "                   <tr><td>MaxPasswordAge</td><td align='center'>$($SCPwdPolicy.MaxPasswordAge)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL (> 60 days)</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>MaxPasswordAge</td><td align='center'>$($SCPwdPolicy.MaxPasswordAge)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # MinPasswordAge (fail if < 1 day)
        $SCMinPwdDays = $SCPwdPolicy.MinPasswordAge.TotalDays
        if ($SCMinPwdDays -lt 1) {
            Add-Content $report "                   <tr><td>MinPasswordAge</td><td align='center'>$($SCPwdPolicy.MinPasswordAge)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL (< 1 day)</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>MinPasswordAge</td><td align='center'>$($SCPwdPolicy.MinPasswordAge)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # PasswordHistoryCount (fail if <= 24)
        if ($SCPwdPolicy.PasswordHistoryCount -le 24) {
            Add-Content $report "                   <tr><td>PasswordHistoryCount</td><td align='center'>$($SCPwdPolicy.PasswordHistoryCount)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL (<= 24)</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>PasswordHistoryCount</td><td align='center'>$($SCPwdPolicy.PasswordHistoryCount)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # ReversibleEncryptionEnabled (fail if True)
        if ($SCPwdPolicy.ReversibleEncryptionEnabled -eq $true) {
            Add-Content $report "                   <tr><td>ReversibleEncryptionEnabled</td><td align='center'>$($SCPwdPolicy.ReversibleEncryptionEnabled)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>ReversibleEncryptionEnabled</td><td align='center'>$($SCPwdPolicy.ReversibleEncryptionEnabled)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # MinPasswordLength (fail if <= 14)
        if ($SCPwdPolicy.MinPasswordLength -le 14) {
            Add-Content $report "                   <tr><td>MinPasswordLength</td><td align='center'>$($SCPwdPolicy.MinPasswordLength)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL (<= 14)</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>MinPasswordLength</td><td align='center'>$($SCPwdPolicy.MinPasswordLength)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # LockoutThreshold (fail if > 10 or == 0)
        if ($SCPwdPolicy.LockoutThreshold -gt 10 -or $SCPwdPolicy.LockoutThreshold -eq 0) {
            Add-Content $report "                   <tr><td>LockoutThreshold</td><td align='center'>$($SCPwdPolicy.LockoutThreshold)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL (> 10 or 0)</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>LockoutThreshold</td><td align='center'>$($SCPwdPolicy.LockoutThreshold)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # LockoutObservationWindow (fail if <= 15 min)
        $SCLockObs = $SCPwdPolicy.LockoutObservationWindow.TotalMinutes
        if ($SCLockObs -le 15) {
            Add-Content $report "                   <tr><td>LockoutObservationWindow</td><td align='center'>$($SCPwdPolicy.LockoutObservationWindow)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL (<= 15 min)</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>LockoutObservationWindow</td><td align='center'>$($SCPwdPolicy.LockoutObservationWindow)</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
        }

        # DistinguishedName (informational)
        Add-Content $report "                   <tr><td>DistinguishedName</td><td align='center' colspan='2'>$($SCPwdPolicy.DistinguishedName)</td></tr>"

        Add-Content $report "               </table>"
        Add-Content $report "           </CENTER>"
    }
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Password Policy failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Password Policy collection failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 4 - Tombstone & Backup
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Collecting Tombstone and Backup Info")
        $SCRootDSE       = Get-ADRootDSE -Server $DCtoConnect -ErrorAction Stop
        $SCConfigNC      = $SCRootDSE.configurationNamingContext
        $SCTombstoneObj  = Get-ADObject -Identity "CN=Directory Service,CN=Windows NT,CN=Services,$SCConfigNC" -Partition $SCConfigNC -Properties tombstoneLifetime -Server $DCtoConnect -ErrorAction Stop
        [int]$SCTombstone = $SCTombstoneObj.tombstoneLifetime
        if ($SCTombstone -eq 0) { $SCTombstone = 60 }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Tombstone Lifetime and Partition Backup</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Item</th><th>Value</th></tr>"
        Add-Content $report "                   <tr><td>Tombstone Lifetime (days)</td><td align='center'>$SCTombstone</td></tr>"

        [string[]]$SCPartitions = $SCRootDSE.namingContexts
        $SCContextType = [System.DirectoryServices.ActiveDirectory.DirectoryContextType]::Domain
        $SCContext = New-Object System.DirectoryServices.ActiveDirectory.DirectoryContext($SCContextType, $($SCDomainInfo.DNSRoot))
        $SCDomCtrl = [System.DirectoryServices.ActiveDirectory.DomainController]::findOne($SCContext)
        ForEach ($SCPartition in $SCPartitions) {
            Try {
                $SCMeta      = $SCDomCtrl.GetReplicationMetadata($SCPartition)
                $SCDsaSig    = $SCMeta.Item("dsaSignature")
                $SCBackupDate = $SCDsaSig.LastOriginatingChangeTime.ToShortDateString()
                Add-Content $report "                   <tr><td>Last backup of '$SCPartition'</td><td align='center'>$SCBackupDate</td></tr>"
            }
            Catch {
                Add-Content $report "                   <tr><td>Last backup of '$SCPartition'</td><td align='center'>N/A</td></tr>"
            }
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Tombstone/Backup failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Tombstone/Backup collection failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 5 - Kerberos Unconstrained Delegation
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Kerberos Unconstrained Delegation")
        [array]$SCKerbDelObjects = Get-ADObject -Filter { (UserAccountControl -BAND 0x0080000) -AND (PrimaryGroupID -ne '516') -AND (PrimaryGroupID -ne '521') } -Server $DCtoConnect -Properties Name,ObjectClass,UserAccountControl,ServicePrincipalName -ErrorAction Stop

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Kerberos Unconstrained Delegation</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>ObjectClass</th><th>Count</th></tr>"

        if ($SCKerbDelObjects.Count -gt 0) {
            $SCKerbGrouped = $SCKerbDelObjects | Group-Object ObjectClass
            foreach ($SCKerbGroup in $SCKerbGrouped) {
                $SCObNames = ($SCKerbGroup.Group | ForEach-Object { $_.Name }) -join '<br>'
                Add-Content $report "                   <tr><td>$($SCKerbGroup.Name)</td><td bgcolor='$TableErrorColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$SCObNames')""><font color='$TableFontOnError'>$($SCKerbGroup.Count)</font></a></td></tr>"
            }
        } else {
            Add-Content $report "                   <tr><td>No unconstrained delegation objects found</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Kerberos Delegation failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Kerberos Delegation collection failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 6 - SYSVOL GPP Passwords
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Scanning SYSVOL for GPP Passwords")
        $SCDomainDNS      = $SCDomainInfo.DNSRoot
        $SCSysVolPath     = "\\$SCDomainDNS\SYSVOL\$SCDomainDNS\Policies\"
        [int]$SCGPPCount  = 0
        $SCGPPFiles       = ""
        $SCGPPFlag        = $TableSuccessColor
        Get-ChildItem $SCSysVolPath -Filter *.xml -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
            if (Select-String -Path $_.FullName -Pattern "Cpassword" -ErrorAction SilentlyContinue) {
                $SCGPPFiles += $_.FullName + "<br>"
                $SCGPPCount++
                $SCGPPFlag = $TableErrorColor
            }
        }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>SYSVOL GPP Password Scan</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Item</th><th>Result</th></tr>"

        if ($SCGPPCount -gt 0) {
            Add-Content $report "                   <tr><td>Cpassword entries found in SYSVOL XML files</td><td bgcolor='$TableErrorColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$SCGPPFiles')""><font color='$TableFontOnError'>$SCGPPCount</font></a></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>Cpassword entries found in SYSVOL XML files</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check SYSVOL GPP scan failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>SYSVOL GPP scan failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 7 - KRBTGT Account
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking KRBTGT Account")
        $SCKrbtgt = Get-ADUser 'krbtgt' -Server $DCtoConnect -Properties 'msds-keyversionnumber',Created,PasswordLastSet -ErrorAction Stop
        $SCKrbtgtAge = (New-TimeSpan -Start $SCKrbtgt.PasswordLastSet -End (Get-Date)).Days

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>KRBTGT Account</h3>"
        Add-Content $report "               <table width='70%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Attribute</th><th>Value</th></tr>"
        Add-Content $report "                   <tr><td>DistinguishedName</td><td align='center'>$($SCKrbtgt.DistinguishedName)</td></tr>"
        Add-Content $report "                   <tr><td>Enabled</td><td align='center'>$($SCKrbtgt.Enabled)</td></tr>"
        Add-Content $report "                   <tr><td>msds-keyversionnumber</td><td align='center'>$($SCKrbtgt.'msds-keyversionnumber')</td></tr>"

        if ($SCKrbtgtAge -gt 180) {
            Add-Content $report "                   <tr><td>PasswordLastSet</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>$($SCKrbtgt.PasswordLastSet) - $SCKrbtgtAge days ago - reset recommended</font></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>PasswordLastSet</td><td bgcolor='$TableSuccessColor' align='center'>$($SCKrbtgt.PasswordLastSet) - $SCKrbtgtAge days ago</td></tr>"
        }

        Add-Content $report "                   <tr><td>Created</td><td align='center'>$($SCKrbtgt.Created)</td></tr>"
        Add-Content $report "               </table>"
        Add-Content $report "           </CENTER>"
    }
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check KRBTGT failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>KRBTGT collection failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 8 - Privileged Groups
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Privileged Groups")
        $SCPrivGroups = @(
            'Administrators',
            'Domain Admins',
            'Enterprise Admins',
        ...
        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Privileged AD Groups</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Group Name</th><th>Member Count</th></tr>"

        foreach ($SCGroup in $SCPrivGroups) {
            Try {
                $SCGrpMembers = Get-ADGroupMember -Identity $SCGroup -Recursive -Server $DCtoConnect -ErrorAction SilentlyContinue | Select-Object SamAccountName,distinguishedName
                $SCGrpMemberNames = ""
                $SCGrpMembers | ForEach-Object { $SCGrpMemberNames += $_.SamAccountName + " (" + $_.distinguishedName + ")<br>" }
                $SCGrpCount = $SCGrpMembers.Count
                if ($null -eq $SCGrpCount) { $SCGrpCount = 0 }
                Add-Content $report "                   <tr><td>$SCGroup</td><td align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$SCGrpMemberNames')"">$SCGrpCount</a></td></tr>"
            }
            Catch {
                Add-Content $report "                   <tr><td>$SCGroup</td><td align='center'>N/A</td></tr>"
            }
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Privileged Groups failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Privileged Groups collection failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 9 - Built-in Administrator Account
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Built-in Administrator Account")
        # SID ending in -500 is always the built-in Administrator
        $SCBuiltinAdmin = Get-ADUser -Filter { adminCount -eq 1 } -Properties SamAccountName,Enabled,LastLogonDate,PasswordLastSet,SID -Server $DCtoConnect -ErrorAction Stop |
                          Where-Object { $_.SID -like '*-500' }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Built-in Administrator Account (SID -500)</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Attribute</th><th>Value</th><th>Status</th></tr>"

        if ($null -ne $SCBuiltinAdmin) {
            # Renamed check (should NOT be named 'Administrator')
            if ($SCBuiltinAdmin.SamAccountName -eq 'Administrator') {
                Add-Content $report "                   <tr><td>Account Name (rename recommended)</td><td align='center'>$($SCBuiltinAdmin.SamAccountName)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>NOT RENAMED</font></td></tr>"
            } else {
                Add-Content $report "                   <tr><td>Account Name (renamed)</td><td align='center'>$($SCBuiltinAdmin.SamAccountName)</td><td bgcolor='$TableSuccessColor' align='center'>RENAMED</td></tr>"
            }
            # Enabled check (should be disabled for day-to-day use)
            if ($SCBuiltinAdmin.Enabled -eq $true) {
                Add-Content $report "                   <tr><td>Enabled (should be disabled)</td><td align='center'>$($SCBuiltinAdmin.Enabled)</td><td bgcolor='$TableMeadiumColor' align='center'>ACTIVE</td></tr>"
            } else {
                Add-Content $report "                   <tr><td>Enabled</td><td align='center'>$($SCBuiltinAdmin.Enabled)</td><td bgcolor='$TableSuccessColor' align='center'>DISABLED</td></tr>"
            }
            $SCAdminLastLogon = if ($SCBuiltinAdmin.LastLogonDate) { $SCBuiltinAdmin.LastLogonDate } else { 'Never' }
            Add-Content $report "                   <tr><td>Last Logon</td><td align='center' colspan='2'>$SCAdminLastLogon</td></tr>"
            Add-Content $report "                   <tr><td>Password Last Set</td><td align='center' colspan='2'>$($SCBuiltinAdmin.PasswordLastSet)</td></tr>"
        } else {
            Add-Content $report "                   <tr><td colspan='3' align='center'>Built-in Administrator account not found or inaccessible.</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Admin Account failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Built-in Administrator check failed: $($_.Exception.Message)</p>"
    }
    ...
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Guest Account")
        $SCGuest = Get-ADUser -Filter { SID -like '*-501' } -Properties Enabled,LastLogonDate -Server $DCtoConnect -ErrorAction Stop

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Guest Account (SID -501)</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Attribute</th><th>Value</th><th>Status</th></tr>"

        if ($null -ne $SCGuest) {
            if ($SCGuest.Enabled -eq $true) {
                Add-Content $report "                   <tr><td>Enabled (should be disabled)</td><td align='center'>True</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>FAIL - Disable immediately</font></td></tr>"
            } else {
                Add-Content $report "                   <tr><td>Enabled</td><td align='center'>False</td><td bgcolor='$TableSuccessColor' align='center'>PASS</td></tr>"
            }
            $SCGuestLastLogon = if ($SCGuest.LastLogonDate) { $SCGuest.LastLogonDate } else { 'Never' }
            Add-Content $report "                   <tr><td>Last Logon</td><td align='center' colspan='2'>$SCGuestLastLogon</td></tr>"
        } else {
            Add-Content $report "                   <tr><td colspan='3' align='center'>Guest account not found.</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Guest Account failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Guest Account check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 11 - Stale Computer Accounts
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Stale Computer Accounts")
        $SCStaleDate = (Get-Date).AddDays(-180)
        [array]$SCAllComputers   = Get-ADComputer -Filter * -Properties LastLogonDate,Enabled,OperatingSystem -Server $DCtoConnect -ErrorAction Stop
        [array]$SCStaleComputers = $SCAllComputers | Where-Object { $_.Enabled -eq $true -and ($_.LastLogonDate -le $SCStaleDate -or $null -eq $_.LastLogonDate) }
        [array]$SCDisabledComps  = $SCAllComputers | Where-Object { $_.Enabled -eq $false }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Computer Accounts</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Metric</th><th>Count</th></tr>"
        Add-Content $report "                   <tr><td>Total Computers</td><td align='center'>$($SCAllComputers.Count)</td></tr>"
        Add-Content $report "                   <tr><td>Disabled Computers</td><td align='center'>$($SCDisabledComps.Count)</td></tr>"

        if ($SCStaleComputers.Count -gt 0) {
            $SCStaleNames = ($SCStaleComputers | ForEach-Object { $_.Name + ' (' + $_.OperatingSystem + ')' }) -join '<br>'
            Add-Content $report "                   <tr><td>Stale Enabled Computers (no logon > 180 days)</td><td bgcolor='$TableMeadiumColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$SCStaleNames')"">$($SCStaleComputers.Count)</a></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>Stale Enabled Computers (no logon > 180 days)</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }

        # OS breakdown
        $SCOSGroups = $SCAllComputers | Where-Object { $_.Enabled -eq $true } | Group-Object OperatingSystem | Sort-Object Count -Descending
        Add-Content $report "               </table>"
        Add-Content $report "               <h3>Active Computer OS Breakdown</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Operating System</th><th>Count</th></tr>"
        foreach ($SCOSGroup in $SCOSGroups) {
            $SCOS = if ($SCOSGroup.Name) { $SCOSGroup.Name } else { 'Unknown/Not Set' }
            # Flag EOL operating systems
            $SCOSFlag = ''
            if ($SCOS -match '2003|2008|2000|XP|Vista|Windows 7') {
                Add-Content $report "                   <tr><td>$SCOS <em>(EOL)</em></td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>$($SCOSGroup.Count)</font></td></tr>"
            } elseif ($SCOS -match '2012') {
                Add-Content $report "                   <tr><td>$SCOS</td><td bgcolor='$TableMeadiumColor' align='center'>$($SCOSGroup.Count)</td></tr>"
            } else {
                Add-Content $report "                   <tr><td>$SCOS</td><td align='center'>$($SCOSGroup.Count)</td></tr>"
            }
        }
        Add-Content $report "               </table>"
        Add-Content $report "           </CENTER>"
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Computer Accounts failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Computer Accounts check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 12 - Protected Users Group
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Protected Users Group")
        $SCPrivGroupNames = @('Domain Admins','Enterprise Admins','Schema Admins','Administrators')
        [array]$SCProtectedMembers = Get-ADGroupMember -Identity 'Protected Users' -Recursive -Server $DCtoConnect -ErrorAction Stop | Select-Object SamAccountName,distinguishedName

        # Get all privileged accounts (union of above groups)
        $SCPrivAccounts = @()
        foreach ($SCPName in $SCPrivGroupNames) {
            Try { $SCPrivAccounts += (Get-ADGroupMember -Identity $SCPName -Recursive -Server $DCtoConnect -ErrorAction SilentlyContinue).SamAccountName } Catch {}
        }
        $SCPrivAccounts = $SCPrivAccounts | Sort-Object -Unique

        $SCNotProtected = $SCPrivAccounts | Where-Object { $_ -notin $SCProtectedMembers.SamAccountName }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Protected Users Group</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Metric</th><th>Value</th></tr>"
        Add-Content $report "                   <tr><td>Members in Protected Users</td><td align='center'>$($SCProtectedMembers.Count)</td></tr>"

        if ($SCNotProtected.Count -gt 0) {
            $SCNotProtNames = $SCNotProtected -join '<br>'
            Add-Content $report "                   <tr><td>Privileged Accounts NOT in Protected Users</td><td bgcolor='$TableMeadiumColor' align='center'><a href='javascript:void(0)' onclick=""window.open('','_blank').document.write('$SCNotProtNames')"">$($SCNotProtected.Count)</a></td></tr>"
        } else {
            Add-Content $report "                   <tr><td>Privileged Accounts NOT in Protected Users</td><td bgcolor='$TableSuccessColor' align='center'>0</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Protected Users failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Protected Users check failed (group may not exist in older domain levels): $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 13 - Fine-Grained Password Policies (PSOs)
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Fine-Grained Password Policies")
        [array]$SCPSOs = Get-ADFineGrainedPasswordPolicy -Filter * -Server $DCtoConnect -ErrorAction Stop

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Fine-Grained Password Policies (PSOs)</h3>"
        Add-Content $report "               <table width='70%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>PSO Name</th><th>Precedence</th><th>Min Length</th><th>Max Age</th><th>Complexity</th><th>Applies To</th></tr>"

        if ($SCPSOs.Count -gt 0) {
            foreach ($SCPSO in $SCPSOs) {
                $SCPSOApplies = ($SCPSO.AppliesTo | ForEach-Object { $_ -replace '.*CN=([^,]+).*','$1' }) -join ', '
                $SCPSOComplex = if ($SCPSO.ComplexityEnabled) { 'Yes' } else { 'No' }
                Add-Content $report "                   <tr><td>$($SCPSO.Name)</td><td align='center'>$($SCPSO.Precedence)</td><td align='center'>$($SCPSO.MinPasswordLength)</td><td align='center'>$($SCPSO.MaxPasswordAge)</td><td align='center'>$SCPSOComplex</td><td>$SCPSOApplies</td></tr>"
            }
        } else {
            Add-Content $report "                   <tr><td colspan='6' align='center'>No Fine-Grained Password Policies defined.</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check PSOs failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Fine-Grained Password Policy check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 14 - AD Replication Health
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking AD Replication Health")
        [array]$SCReplPartners = Get-ADReplicationPartnerMetadata -Target * -Scope Domain -Server $DCtoConnect -ErrorAction Stop

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>AD Replication Health</h3>"
        Add-Content $report "               <table width='80%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Server</th><th>Partner</th><th>Last Replication</th><th>Consecutive Failures</th><th>Last Error</th></tr>"

        foreach ($SCRepl in $SCReplPartners) {
            $SCReplDate   = if ($SCRepl.LastReplicationSuccess) { $SCRepl.LastReplicationSuccess } else { 'Never' }
            $SCReplFails  = $SCRepl.ConsecutiveReplicationFailures
            $SCReplErr    = if ($SCRepl.LastReplicationResult -eq 0) { 'None' } else { $SCRepl.LastReplicationResult }
            if ($SCReplFails -gt 0 -or $SCRepl.LastReplicationResult -ne 0) {
                Add-Content $report "                   <tr><td>$($SCRepl.Server)</td><td>$($SCRepl.Partner)</td><td align='center'>$SCReplDate</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>$SCReplFails</font></td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>$SCReplErr</font></td></tr>"
            } else {
                Add-Content $report "                   <tr><td>$($SCRepl.Server)</td><td>$($SCRepl.Partner)</td><td align='center'>$SCReplDate</td><td bgcolor='$TableSuccessColor' align='center'>0</td><td bgcolor='$TableSuccessColor' align='center'>None</td></tr>"
            }
        }

        if ($SCReplPartners.Count -eq 0) {
            Add-Content $report "                   <tr><td colspan='5' align='center'>Only one DC or replication data unavailable.</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Replication Health failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Replication Health check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 15 - DC Operating System Versions
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking DC Operating System Versions")
        [array]$SCDCList = Get-ADDomainController -Filter * -Server $DCtoConnect -ErrorAction Stop

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Domain Controller Operating Systems</h3>"
        Add-Content $report "               <table width='70%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>DC Name</th><th>Operating System</th><th>OS Version</th><th>Status</th></tr>"

        foreach ($SCDC in $SCDCList) {
            $SCDCOs = $SCDC.OperatingSystem
            if ($SCDCOs -match '2003|2008|2000') {
                Add-Content $report "                   <tr><td>$($SCDC.HostName)</td><td>$SCDCOs</td><td align='center'>$($SCDC.OperatingSystemVersion)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>EOL - Upgrade Required</font></td></tr>"
            } elseif ($SCDCOs -match '2012') {
                Add-Content $report "                   <tr><td>$($SCDC.HostName)</td><td>$SCDCOs</td><td align='center'>$($SCDC.OperatingSystemVersion)</td><td bgcolor='$TableMeadiumColor' align='center'>End of Mainstream Support</td></tr>"
            } elseif ($SCDCOs -match '2016|2019|2022|2025') {
                Add-Content $report "                   <tr><td>$($SCDC.HostName)</td><td>$SCDCOs</td><td align='center'>$($SCDC.OperatingSystemVersion)</td><td bgcolor='$TableSuccessColor' align='center'>Supported</td></tr>"
            } else {
                Add-Content $report "                   <tr><td>$($SCDC.HostName)</td><td>$SCDCOs</td><td align='center'>$($SCDC.OperatingSystemVersion)</td><td align='center'>Unknown</td></tr>"
            }
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check DC OS Versions failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>DC OS Version check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 16 - Accounts Expiring in Next 30 Days
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Expiring Accounts")
        $SCExpirySoon = (Get-Date).AddDays(30)
        [array]$SCExpiringAccounts = Get-ADUser -Filter { Enabled -eq $true -and AccountExpirationDate -ge 1 } `
            -Properties SamAccountName,AccountExpirationDate,DisplayName -Server $DCtoConnect -ErrorAction Stop |
            Where-Object { $_.AccountExpirationDate -ne $null -and $_.AccountExpirationDate -le $SCExpirySoon -and $_.AccountExpirationDate -ge (Get-Date) }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Accounts Expiring in the Next 30 Days</h3>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>SamAccountName</th><th>Display Name</th><th>Expiration Date</th></tr>"

        if ($SCExpiringAccounts.Count -gt 0) {
            foreach ($SCExpAcc in ($SCExpiringAccounts | Sort-Object AccountExpirationDate)) {
                Add-Content $report "                   <tr><td>$($SCExpAcc.SamAccountName)</td><td>$($SCExpAcc.DisplayName)</td><td bgcolor='$TableMeadiumColor' align='center'>$($SCExpAcc.AccountExpirationDate.ToShortDateString())</td></tr>"
            }
        } else {
            Add-Content $report "                   <tr><td colspan='3' bgcolor='$TableSuccessColor' align='center'>No accounts expiring in the next 30 days.</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Expiring Accounts failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Expiring Accounts check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 17 - Orphaned adminCount Accounts
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking Orphaned adminCount Accounts")
        # Accounts with adminCount=1 that are NOT actually in any privileged group (SDProp leftovers)
        $SCPrivGroupSIDs = @('Domain Admins','Enterprise Admins','Schema Admins','Administrators',
                             'Account Operators','Server Operators','Backup Operators','Print Operators',
                             'Group Policy Creator Owners','DNSAdmins')
        $SCAllPrivMembers = @()
        foreach ($SCPGName in $SCPrivGroupSIDs) {
            Try { $SCAllPrivMembers += (Get-ADGroupMember -Identity $SCPGName -Recursive -Server $DCtoConnect -ErrorAction SilentlyContinue).SamAccountName } Catch {}
        }
        $SCAllPrivMembers = $SCAllPrivMembers | Sort-Object -Unique

        [array]$SCAdminCountUsers = Get-ADUser -Filter { adminCount -eq 1 } -Properties SamAccountName,adminCount -Server $DCtoConnect -ErrorAction Stop
        [array]$SCOrphanedAdmins  = $SCAdminCountUsers | Where-Object { $_.SamAccountName -notin $SCAllPrivMembers }

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>Orphaned adminCount=1 Accounts</h3>"
        Add-Content $report "               <p class='note'>Accounts with adminCount=1 that are no longer members of any privileged group. These retain relaxed inheritance and should be reviewed - either add them to a privileged group intentionally or clear adminCount and restore ACL inheritance.</p>"
        Add-Content $report "               <table width='60%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>SamAccountName</th><th>Status</th></tr>"

        if ($SCOrphanedAdmins.Count -gt 0) {
            foreach ($SCOrph in $SCOrphanedAdmins) {
                Add-Content $report "                   <tr><td>$($SCOrph.SamAccountName)</td><td bgcolor='$TableMeadiumColor' align='center'>Review Required</td></tr>"
            }
        } else {
            Add-Content $report "                   <tr><td colspan='2' bgcolor='$TableSuccessColor' align='center'>No orphaned adminCount accounts found.</td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Orphaned adminCount failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>Orphaned adminCount check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Section 18 - AD Recycle Bin Status
    #------------------------------------------------------------
    Try {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check: Checking AD Recycle Bin")
        $SCRecycleBin = Get-ADOptionalFeature -Filter { Name -eq 'Recycle Bin Feature' } -Server $DCtoConnect -ErrorAction Stop
        $SCRBEnabled  = ($SCRecycleBin.EnabledScopes.Count -gt 0)

        Add-Content $report "           <CENTER>"
        Add-Content $report "               <h3>AD Recycle Bin</h3>"
        Add-Content $report "               <table width='50%' align='center' border='1'>"
        Add-Content $report "                   <tr><th>Feature</th><th>Status</th></tr>"

        if ($SCRBEnabled) {
            Add-Content $report "                   <tr><td>AD Recycle Bin</td><td bgcolor='$TableSuccessColor' align='center'>Enabled</td></tr>"
        } else {
            Add-Content $report "                   <tr><td>AD Recycle Bin (should be enabled)</td><td bgcolor='$TableErrorColor' align='center'><font color='$TableFontOnError'>NOT Enabled</font></td></tr>"
        }
    ...
    Catch {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Security Check Recycle Bin failed: "+$_.Exception.Message)
        Add-Content $report "           <p class='note'>AD Recycle Bin check failed: $($_.Exception.Message)</p>"
    }

    #------------------------------------------------------------
    # Footer for Security Check tab
    #------------------------------------------------------------
    Add-Content $report "           <footer>"
    Add-Content $report "               <a href='#top' class=back-button>Back to the top</a>"
    Add-Content $report "               <p class=disclaimer><strong>Disclaimer:</strong> This report was generated by the AD Health Report script. Information is provided for informational purposes only. The Tech Samurai.</p>"
    Add-Content $report "           </footer>"
    Add-Content $report "       </div>"

    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Security Check Report complete")

}

#----------------------------------------[End of SecurityCheckReport Function]---------------------------------------------------
...
Add-Content $report "       <title>AD Health Report</title>"
...
Add-Content $report "               background-color: #8B0000;"
...
Add-Content $report "               width: 16.66%;"
Add-Content $report "           }"
Add-Content $report ""
...
Add-Content $report ""
Add-Content $report "           <h1><span class='title'>Active Directory Health Report</span></h1>" 
$button = @'
...
Add-Content $report $button
$button = @'
            <button class="tablink" onclick="openTab('SecurityCheck')">Security Check</button>
'@
Add-Content $report $button

Add-Content $report "           <p class='note'>Active Directory Health Report - Powered by The Tech Samurai</p>"
Add-Content $report "       </header>" 
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Forest Report: "+$Forest)
...
Try
    {
        ...
        $Fore = Import-Clixml -Path C:\ADHealthReport\Hammer\Forest.xml
        ...
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - RecycleBin status: "+$RecycleBin)
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Forest Reporting phase.")
    }
Catch 
    { 
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors found -------------")
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during catcher: "+$_.Exception.Message)
    }
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Trust reporting")
...
if($Trust) 
    { 
                ...
                Foreach ($Trusts in $Trust)
                    {
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Trust Found for: "+$T3Source+ " To "+$T3Target)
        ...
        Catch 
            { 
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors found -------------")
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
            }
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of TRUST Report phase.")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domains Reporting")
...
Try
    {
        ...
        Foreach ($Domain0 in $Global:DomainNames)
            {
                $Domain1 = Import-Clixml -Path ('C:\ADHealthReport\Hammer\Domain_'+$Domain0+'.xml')
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting the following domain: "+$D2Name)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Domain phase.")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting SysVol Reporting")
...
Foreach ($Domain in $Global:DomainNames) 
    {
        Try 
            {
                $Domain2 = Import-Clixml -Path ('C:\ADHealthReport\Hammer\Domain_'+$Domain+'.xml')
                        ...
                        if ($SYSSIZE -ge 0.01)
                            {
                                ...
                                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - "+$SYSEXT+" extension found, total of: "+$SYSCOUNT+" files ("+$SYSSIZE+")")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of SYSVol Reporting")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting User Accounts Reporting")
...
Foreach ($Domain in $Global:DomainNames)
    {
        Try
            {
                ...
                $Usrs = Import-Clixml -Path ('C:\ADHealthReport\Hammer\Domain_'+$Domain+'.xml')
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Inventoring User Accounts in the Domain: "+$UsDomain)
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Total users found: "+$AllUsers)
        ...
        Catch 
            { 
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors were found  -------------")
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
            }
    }


Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - User Accounts Reporting finished")
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of User Account phase.")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Computer Accounts reporting")
        ...
        Try
            {
                ...
                $PC =  Import-Clixml -Path ('C:\ADHealthReport\Hammer\Domain_'+$Domain+'.xml')
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting Computer Accounts in the Domain: "+$Domain)
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Total Computers found: "+$PCAllC)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Computer Accounts Reporting finished")
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Computer Account phase.")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Groups Reporting")
...
Foreach ($Domain in $Global:DomainNames) 
    {
        Try
            {
                $Grp =  Import-Clixml -Path ('C:\ADHealthReport\Hammer\Domain_'+$Domain+'.xml')
                ...
                Foreach ($gp in $Groups)
                    {
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Inventoring Group: "+$GName)
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Total members found: "+$GCounter)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Domain Groups Inventory finished")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Group Policy Objects Reporting")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting GPOs Reporting")
        ...
        Try
            {
                ...
                [xml]$XmlDocument = Get-Content -Path ('C:\ADHealthReport\Hammer\GPOs_'+$Domain+'.XML')
                ...
                Foreach ($gpo in $gpos)
                    {
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Inventoring the Following GPO: "+$GpoName)
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - GPOs Inventory finished")
            ...
            Catch 
                { 
                    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors were found during the GPO Inventoring -------------")
                    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
                }
            }
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Reporting")
...
foreach ($DC in $Global:DCs)
    {
        Try
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Start Reporting of: "+$DC)

                $DCD = Import-Clixml -Path ('C:\ADHealthReport\Hammer\Inv_'+$DC+'.xml')    
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting Operating System Version of: "+$DCHostName)
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting FSMO of: "+$DCHostName)
                Add-Content $report "                       <td align=center>$FSMO</td>" 
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting Site of: "+$DCHostName)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Domain Controllers Reporting finished")
Add-Content $report "               </table>" 
Add-Content $report "           </CENTER>"
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting NTP Reporting")
...
foreach ($DC in $Global:DCs)
    {
        Try 
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Start NTP Reporting of: "+$DC)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - NTP Reporting finished")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DNS Server Reporting")
        ...
        Try 
            {
                ...
                if(![string]::IsNullOrEmpty($DNS))
                    {          
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting DNS Server: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Validating DNS Server: "+$DNSName)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - DNS Servers Reporting finished")
                ...
                Try 
                    {
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Reporting of: "+$DC.Name)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag initial validation: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag VerifyReference Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Advertising Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag FrsEvent: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag DFSREvent Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag SysvolCheck Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag KccEvent Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag KnowsOfRoleHolders Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag MachineAccount Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag NCSecDesc Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag NetLogons Test: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag ObjectsReplicated: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Replications: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag RIDManager: "+$DC)
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag SystemLog Test: "+$DC)
                ...
                Try 
                    {
                        ...
                        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Reporting of: "+$DC)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Checking if RSOP Folder already exists.")    

Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Security Log Reporting.")   
...
foreach ($DC in $Global:DCs)
    ...
    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Event Viewer Log Inventory of:"+$DC) 
    ...
    Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Log sizes adquired:"+$SysLogSize+" , "+$SecLogSize+" and "+$ADLogSize) 
    ...
    Add-Content $report "<td align=center>$SecRec</td>" 
        ...
        Add-Content $report "</tr>" 
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of server:"+$DC) 
}
Catch{
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during catcher: "+$_.Exception.Message) 
}
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Backup Reporting.")   
...
foreach ($DC in $Global:DCs) 
    {
        Try
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Reporting of Active Backups:"+$DC)
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Latest backup on: '+$DCHostname +'. was on:"+$Backup)
                ...
                Add-Content $report "                   </tr>" 
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of BackUp for server:"+$DC)
            }
        Catch
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Print Spooler Reporting.")   
...
foreach ($DC in $Global:DCs) 
    {
        Try
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Print Spooler Reporting of:"+$DC) 
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Print Spooler Status:"+$State)
                
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Print Spooler Startup Mode:"+$Startup) 
                ...
                Add-Content $report "                   </tr>" 
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Print Spooler Reporting for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message) 
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Hotfix Reporting.")   
...
foreach ($DC in $Global:DCs)
    {
        Try
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining HotFix Inventory of:"+$DC) 
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Latest Hotfix installed on:"+$HFDate)
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Latest Hotfix installed:"+$HFID) 
                ...
                Add-Content $report "                   </tr>" 
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Hotfix Inventory for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during catcher: "+$_.Exception.Message) 
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controller Security Policies Reporting") 
        ...
        Try 
            {
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reading RSOP result for: " +$DC) 
                [xml]$XmlDocument = Get-Content -Path ("C:\ADHealthReport\Hammer\RSOP_"+$DC+".xml")
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controller User Rights Assignments reporting") 
        ...
        Try 
            {
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controller User Rights Assignments on the server: "+$DC) 
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Ending Domain Controller User Rights Assignments for: "+$DC)
...
Add-Content $report "           </CENTER>"
Add-Content $report "           <CENTER>"
Add-Content $report "               <p class='note'>According to Microsoft the built-in Administrator Account should be denied logon through network, Remote Desktop, as a batch job and as a service in all Domain Controllers in the environment.</p>" 
Add-Content $report "           </CENTER>"
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Hardware Reporting.")   
...
Foreach($DC in $Global:DCs) 
    {
        Try
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Hardware Reporting of:"+$DC) 
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Hardware Reporting for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message) 
...
Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Installed Software Reporting.")   

Add-Content $report "               <section>"
Add-Content $report "                   <h2>Domain Controller's Software Inventory<HR></h2>" 
Add-Content $report "                   <p class='note'>Software maintenance is critical for a safe and secure environment. Keep installed software at the bare minimum needed on Domain Controllers.</p>" 
Add-Content $report "                   <p class='note'>Keep required software up to date and check regularly for critical vulnerabilities.</p>" 
Add-Content $report "               </section>"
Add-Content $report "           <CENTER>"
        ...
        Try
            {
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Software Reporting of:"+$DC) 
                ...
                Add-Content $report "                           <th width='8%' align='center'>Domain</th>" 
                Add-Content $report "                           <th width='15%' align='center'>Domain Controller</th>" 
                Add-Content $report "                           <th width='25%' align='center'>Installed Software</th>" 
                        ...
                        If ($sw.DisplayName)
                            {
                                ...
                                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Reporting of:"+$SWD)
                        ...
                        If($sw.DisplayName)
                            {
                                ...
                                Add-Content $report "                           <td align=center>x86</td>"
                                Add-Content $report "                           <td align=center>$SWDV</td>" 
                ...
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Software Reporting for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during Reporting: "+$_.Exception.Message) 
...
Add-Content $report "               </CENTER>"
Add-Content $report "               <p class='note'>Make sure that the software listed above is really necessary on those servers. If not needed, uninstall it from the Domain Controllers as soon as possible. </p>" 
...
if($Global:Option -eq 1 -or $Global:Option -eq 2 -or $Global:Option -eq 3 -or $Global:Option -eq 4)
    {
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Forest Pre Inventory")
        $Global:Forest = [system.directoryservices.activedirectory.Forest]::GetCurrentForest()
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domains Pre Inventory")
        $Global:Domains = $Global:Forest.domains
        $Global:DomainNames = $Global:Domains.name
        Add-Content $ADHRLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Pre Inventory")
...
elseif($Global:Option -eq 5)
    {
        ...
        Compress-Archive -Path 'C:\ADHealthReport\Hammer' -DestinationPath 'C:\ADHealthReport\ADHealthReport.zip'
...
elseif($Global:Option -eq 6)
    {
        ...
        $DomainControllers = Get-ChildItem -Path 'C:\ADHealthReport\Hammer\' -Recurse
...
elseif($Global:Option -eq 5)
    {
        ...
        Write-Host 'C:\ADHealthReport\ADHealthReport.zip' -BackgroundColor Red
    }
