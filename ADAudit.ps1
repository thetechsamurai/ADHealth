...
<#
.SYNOPSIS
Active Directory audit Inventory
...
(https://github.com/clinthuffman/PAL), Claudio Merola and Raphaela Pereira, and Microsoft's Ned Pyle blogpost 'What does DCDIAG actually... do?'
https://blogs.technet.microsoft.com/askds/2011/03/22/what-does-dcdiag-actually-do/ 

.OUTPUTS
Details regarding the environment will be presented during the execution of the script. The log file will be created at: C:\Adaudit\ADaudit.log
...
Write-Host 'Starting ADaudit Script..' -ForegroundColor Green
...
if ((Test-Path -Path C:\ADaudit -PathType Container) -eq $false) {New-Item -Type Directory -Force -Path C:\ADaudit}

$Global:report = ("C:\ADaudit\ADaudit_Report_"+(get-date -Format 'yyyy-MM-dd-hh-mm')+".htm") 
if ((test-path $report) -eq $false) {new-item $report -Type file -Force}
Clear-Content $report 

$Global:ADauditLog = "C:\ADaudit\ADaudit.log"
if ((test-path $ADauditLog) -eq $false) {new-item $ADauditLog -Type file -Force}
Clear-Content $ADauditLog 

Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting ADaudit Script")
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Data Catcher")
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Setting Error Action Preference")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Selecting Script Option")
Write-Host ""
...
Write-Host ""
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Option: "+$Global:Option+" Selected")

#----------------------------------------[Begin of Sword]---------------------------------------------------

function Sword 
    {
        Write-Host 'Sharpening The Sword..'
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting The Sword!")
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Creating Sword Folder")


        if ((Test-Path -Path C:\ADaudit\Sword -PathType Container) -eq $false) {New-Item -Type Directory -Force -Path C:\ADaudit\Sword}

        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Cleaning current Powershell Job History")

        Get-Job | Remove-Job

        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Calling DCDiag")

        if($Clear.isPresent)
            {
                $Files = Get-ChildItem -path 'C:\ADaudit\Sword\' 
        ...
        function SwordForest 
            {
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Active Directory RecycleBin Check")
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Duplicated SPNs check")

                Start-job -Name 'SPN' -scriptblock {setspn -X -F} | Out-Null

                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Trusts Inventory")
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Inventory")
        ...
        function SwordDomain
            {
                ...
                Foreach ($Domain in $Global:Domains)
                    { 
                        ...
                        start-job -Name ($Domain.Name+'_GPOs') -scriptblock {Get-GPOReport -All -ReportType XML -Path ("C:\ADaudit\Sword\GPOs_"+$args+".xml")} -ArgumentList $Domain.Name | Out-Null
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Inventory")
            }        
        ...
        function SwordDC
            {
                ...
                #start-job -Name ($DC.Name+'_CleartxtEvt') -scriptblock {(Get-EventLog -LogName Security -InstanceId 4624 -Message '*Logon Type:			8*' -ComputerName $args).Count} -ArgumentList $DC.Name | Out-Null
                    ...
                    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Inventory of: "+$DC.Name+'. On: '+$DC.Domain)
                    ...
                    $GPResult = ([PowerShell]::Create()).AddScript({param($DomControl)Get-GPResultantSetOfPolicy -Computer $DomControl -ReportType Xml -Path ("C:\ADaudit\Sword\RSOP_"+$DomControl+".xml")}).AddArgument($($args[0]))
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Waiting Inventories Conclusion")
            }        
                ...
                while (get-job | Where-Object {$_.State -eq 'Running'})
                    {
                        ...
                        if ((New-TimeSpan -Start $WaitTime2 -End (get-date)).TotalMinutes -ge 10)
                            {
                                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Still Waiting for the following Inventory Jobs:")
                                foreach($jbb in ($jb | Where-Object {$_.State -eq 'Running'}))
                                    {
                                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Job: "+$jbb.Name)
                                    }
                                $WaitTime2 = get-date
                            }
                        if ((New-TimeSpan -Start $WaitTime -End (get-date)).TotalMinutes -ge $JobTimeout)
                            {
                                ...
                                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - Timing Out Inventory Jobs")
                            }
                        Start-Sleep -Seconds 2
                    }
                Write-Progress -activity 'Running Inventories' -Status "100% Complete." -Completed

                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - All Inventories are completed")
        ...
        function ForestJob
            {
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Forest Inventory")
                ...
                if ((test-path 'C:\ADaudit\Sword\Forest.xml') -eq $true) {remove-item -Path 'C:\ADaudit\Sword\Forest.xml' -Force}
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Registering Forest XML File")

                $Fores | Export-Clixml -Path 'C:\ADaudit\Sword\Forest.xml'
        ...
        function DomainJob 
            {    
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Domain Inventory")
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Domains Details")
                ...
                Foreach ($Domain in $Global:Domains)
                    {
                        ...
                        Start-job -Name ($Domain.Name+'_job') -ScriptBlock {                            
                            if((test-path ('C:\ADaudit\Sword\Domain_'+$($args[0]).Name+'.xml')) -eq $true -and $($args[9]) -ne 2) 
                                {
                                    remove-item -Path ('C:\ADaudit\Sword\Domain_'+$($args[0]).Name+'.xml') -Force
                                }
                            ...
                            $DomainTable | Export-Clixml -Path ('C:\ADaudit\Sword\Domain_'+$($args[0]).Name+'.xml')
        ...
        function DCjob 
            {
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting to Process Domain Controllers Inventory")
                ...
                Foreach ($DC in $Global:DCs)
                    {
                        ...
                        Start-job -Name ('Job_'+$DC.Name) -ScriptBlock {
                            if((test-path ("C:\ADaudit\Sword\Inv_"+$($args[0]).Name+".xml")) -eq $true -and $($args[3]) -ne 2) 
                                {
                                    remove-item -Path ("C:\ADaudit\Sword\Inv_"+$($args[0]).Name+".xml") -Force
                                }
                            ...
                            $DomControl | Export-Clixml -Path ('C:\ADaudit\Sword\Inv_'+$($args[0]).Name+'.xml')
                ...
                while (get-job | Where-Object {$_.State -eq 'Running'})
                    {
                        ...
                        if ((New-TimeSpan -Start $WaitTime2 -End (get-date)).TotalMinutes -ge 10)
                            {
                                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Still Waiting for the following Processing Jobs:")
                                foreach($jbb in ($jb | Where-Object {$_.State -eq 'Running'}))
                                    {
                                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Warn - Job: "+$jbb.Name)
                                    }
    ...
    {
    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - The following Inventory Job Failed: "+$jbName)
    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - With the following Command Line: "+$jbcmd)
    }
    $time = New-TimeSpan -Start $JB.PSBeginTime -End $JB.PSEndTime
    $TimeJobMin = $time.TotalMinutes.ToString('#######.##')
    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - The following Inventory Job: "+$jbName+'. run time was: '+$TimeJobMin+' Minutes.')
    }

    Write-Host ('End of Sword phase. ') -NoNewline
    ...
    $DomainControllersFolder = Get-ChildItem -Path 'C:\ADaudit\Sword\' -Recurse
    ...
    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Sword")
...
Add-Content $report "       <title>ADaudit - $Forest</title>"
...
Add-Content $report "               background-color: #a90819;"
...
Add-Content $report ""
Add-Content $report "           <h1>Active Directory Audit Report</h1>" 
$button = @'
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Forest Report: "+$Forest)
...
Try
    {
        ...
        $Fore = Import-Clixml -Path C:\ADaudit\Sword\Forest.xml
        ...
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - RecycleBin status: "+$RecycleBin)
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Forest Reporting phase.")
    }
Catch 
    { 
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors found -------------")
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during catcher: "+$_.Exception.Message)
    }
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Trust reporting")
...
if($Trust) 
    { 
                ...
                Foreach ($Trusts in $Trust)
                    {
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Trust Found for: "+$T3Source+ " To "+$T3Target)
        ...
        Catch 
            { 
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors found -------------")
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
            }
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of TRUST Report phase.")
...
Add-Content $report "               <p class=disclaimer><strong>Disclaimer:</strong> This report was generated using the ADaudit Powershell Script. The information provided in this report is provided 'as-is' and is intended for information purposes only. The information present at the script is licensed 'as-is'. You bear the risk of using it. The contributors give no express warranties, guarantees or conditions. You may have additional consumer rights under your local laws which this license cannot change. To the extent permitted under your local laws, the contributors exclude the implied warranties of merchantability, fitness for a particular purpose and non-infringement.</p>"
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domains Reporting")
...
Try
    {
        ...
        Foreach ($Domain0 in $Global:DomainNames)
            {
                $Domain1 = Import-Clixml -Path ('C:\ADaudit\Sword\Domain_'+$Domain0+'.xml')
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting the following domain: "+$D2Name)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Domain phase.")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting SysVol Reporting")
...
Foreach ($Domain in $Global:DomainNames) 
    {
        Try 
            {
                $Domain2 = Import-Clixml -Path ('C:\ADaudit\Sword\Domain_'+$Domain+'.xml')
                        ...
                        if ($SYSSIZE -ge 0.01)
                            {
                                ...
                                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - "+$SYSEXT+" extension found, total of: "+$SYSCOUNT+" files ("+$SYSSIZE+")")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of SYSVol Reporting")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting User Accounts Reporting")
...
Foreach ($Domain in $Global:DomainNames)
    {
        Try
            {
                ...
                $Usrs = Import-Clixml -Path ('C:\ADaudit\Sword\Domain_'+$Domain+'.xml')
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Inventoring User Accounts in the Domain: "+$UsDomain)
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Total users found: "+$AllUsers)
        ...
        Catch 
            { 
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors were found  -------------")
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
            }
    }


Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - User Accounts Reporting finished")
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of User Account phase.")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Computer Accounts reporting")
        ...
        Try
            {
                ...
                $PC =  Import-Clixml -Path ('C:\ADaudit\Sword\Domain_'+$Domain+'.xml')
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting Computer Accounts in the Domain: "+$Domain)
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Total Computers found: "+$PCAllC)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Computer Accounts Reporting finished")
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Computer Account phase.")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Groups Reporting")
...
Foreach ($Domain in $Global:DomainNames) 
    {
        Try
            {
                $Grp =  Import-Clixml -Path ('C:\ADaudit\Sword\Domain_'+$Domain+'.xml')
                ...
                Foreach ($gp in $Groups)
                    {
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Inventoring Group: "+$GName)
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Total members found: "+$GCounter)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Domain Groups Inventory finished")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Group Policy Objects Reporting")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting GPOs Reporting")
        ...
        Try
            {
                ...
                [xml]$XmlDocument = Get-Content -Path ('C:\ADaudit\Sword\GPOs_'+$Domain+'.XML')
                ...
                Foreach ($gpo in $gpos)
                    {
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Inventoring the Following GPO: "+$GpoName)
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - GPOs Inventory finished")
            ...
            Catch 
                { 
                    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - ------------- Errors were found during the GPO Inventoring -------------")
                    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
                }
            }
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Reporting")
...
foreach ($DC in $Global:DCs)
    {
        Try
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Start Reporting of: "+$DC)

                $DCD = Import-Clixml -Path ('C:\ADaudit\Sword\Inv_'+$DC+'.xml')    
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting Operating System Version of: "+$DCHostName)
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting FSMO of: "+$DCHostName)
                Add-Content $report "                       <td align=center>$FSMO</td>" 
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting Site of: "+$DCHostName)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Domain Controllers Reporting finished")
Add-Content $report "               </table>" 
Add-Content $report "           </CENTER>"
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting NTP Reporting")
...
foreach ($DC in $Global:DCs)
    {
        Try 
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Start NTP Reporting of: "+$DC)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - NTP Reporting finished")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DNS Server Reporting")
        ...
        Try 
            {
                ...
                if(![string]::IsNullOrEmpty($DNS))
                    {          
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reporting DNS Server: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Validating DNS Server: "+$DNSName)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - DNS Servers Reporting finished")
                ...
                Try 
                    {
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Reporting of: "+$DC.Name)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag initial validation: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag VerifyReference Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Advertising Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag FrsEvent: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag DFSREvent Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag SysvolCheck Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag KccEvent Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag KnowsOfRoleHolders Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag MachineAccount Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag NCSecDesc Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag NetLogons Test: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag ObjectsReplicated: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Replications: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag RIDManager: "+$DC)
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag SystemLog Test: "+$DC)
                ...
                Try 
                    {
                        ...
                        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting DCDiag Reporting of: "+$DC)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Checking if RSOP Folder already exists.")    

Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Security Log Reporting.")   
...
foreach ($DC in $Global:DCs)
    ...
    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Event Viewer Log Inventory of:"+$DC) 
    ...
    Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Log sizes adquired:"+$SysLogSize+" , "+$SecLogSize+" and "+$ADLogSize) 
    ...
    Add-Content $report "<td align=center>$SecRec</td>" 
        ...
        Add-Content $report "</tr>" 
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of server:"+$DC) 
}
Catch{
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during catcher: "+$_.Exception.Message) 
}
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Backup Reporting.")   
...
foreach ($DC in $Global:DCs) 
    {
        Try
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Reporting of Active Backups:"+$DC)
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Latest backup on: '+$DCHostname +'. was on:"+$Backup)
                ...
                Add-Content $report "                   </tr>" 
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of BackUp for server:"+$DC)
            }
        Catch
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Print Spooler Reporting.")   
...
foreach ($DC in $Global:DCs) 
    {
        Try
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Print Spooler Reporting of:"+$DC) 
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Print Spooler Status:"+$State)
                
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Print Spooler Startup Mode:"+$Startup) 
                ...
                Add-Content $report "                   </tr>" 
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Print Spooler Reporting for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message) 
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Hotfix Reporting.")   
...
foreach ($DC in $Global:DCs)
    {
        Try
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining HotFix Inventory of:"+$DC) 
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Latest Hotfix installed on:"+$HFDate)
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Latest Hotfix installed:"+$HFID) 
                ...
                Add-Content $report "                   </tr>" 
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Hotfix Inventory for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during catcher: "+$_.Exception.Message) 
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controller Security Policies Reporting") 
        ...
        Try 
            {
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Reading RSOP result for: " +$DC) 
                [xml]$XmlDocument = Get-Content -Path ("C:\ADaudit\Sword\RSOP_"+$DC+".xml")
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controller User Rights Assignments reporting") 
        ...
        Try 
            {
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controller User Rights Assignments on the server: "+$DC) 
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Ending Domain Controller User Rights Assignments for: "+$DC)
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Hardware Reporting.")   
...
Foreach($DC in $Global:DCs) 
    {
        Try
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Hardware Reporting of:"+$DC) 
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Hardware Reporting for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during reporting: "+$_.Exception.Message) 
...
Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Domain Controller's Installed Software Reporting.")   
        ...
        Try
            {
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Begining Software Reporting of:"+$DC) 
                        ...
                        If ($sw.DisplayName)
                            {
                                ...
                                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Reporting of:"+$SWD)
                ...
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - End of Software Reporting for server:"+$DC) 
            }
        Catch
            {
                Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Err - The following error ocurred during Reporting: "+$_.Exception.Message) 
...
Write-Host 'ADaudit Version Check..'

$VerValid = Invoke-WebRequest -Uri "https://raw.githubusercontent.com/Merola132/ADaudit/master/Docs/VersionControl" -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -TimeoutSec 5
if ($VerValid.StatusCode -eq 200) 
    {
        if (($VerValid.Content[0]+$VerValid.Content[1]+$VerValid.Content[2]) -eq $Ver) 
            ...
            {
                Write-Host ('Version: '+$Ver+' - This version of ADaudit is outdated. Please access https://github.com/ClaudioMerola/ADaudit for the lastest version and corrections.') -ForegroundColor Red
            }
    }
elseif ($null -eq $VerValid ) 
    {
        Write-Host ('Version: '+$Ver+' - ADaudit version validation was not possible. Please access https://github.com/ClaudioMerola/ADaudit for the lastest version and corrections.') -ForegroundColor Red
    }
...
if($Global:Option -eq 1 -or $Global:Option -eq 2 -or $Global:Option -eq 3 -or $Global:Option -eq 4)
    {
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Forest Pre Inventory")
        $Global:Forest = [system.directoryservices.activedirectory.Forest]::GetCurrentForest()
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domains Pre Inventory")
        $Global:Domains = $Global:Forest.domains
        $Global:DomainNames = $Global:Domains.name
        Add-Content $ADauditLog ((get-date -Format 'MM-dd-yyyy  HH:mm:ss')+" - Info - Starting Domain Controllers Pre Inventory")
...
elseif($Global:Option -eq 5)
    {
        ...
        Compress-Archive -Path 'C:\ADaudit\Sword' -DestinationPath 'C:\ADaudit\ADaudit.zip'
...
elseif($Global:Option -eq 6)
    {
        ...
        $DomainControllers = Get-ChildItem -Path 'C:\ADaudit\Sword\' -Recurse
...
elseif($Global:Option -eq 5)
    {
        ...
        Write-Host 'C:\ADaudit\ADaudit.zip' -BackgroundColor Red
    }
